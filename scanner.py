...
def fetch_all_us_common_stocks() -> List[str]:
    """
    US exchange-listed operating-company common stocks from:
      - Nasdaq
      - NYSE
      - NYSE American
      - NYSE Arca-listed common stocks if classified as common equity

    Excludes:
      - ETFs
      - test issues
      - warrants / units / rights / preferred shares where identifiable
      - symbols with special suffixes that yfinance often cannot resolve cleanly
    """
    ...
    if not nas.empty:
        ...
        for _, row in n.iterrows():
            ...
            name = str(row[name_col]).upper() if name_col else ""
            rows.append((sym, name, "NASDAQ"))

    # NYSE / NYSE American / other listed exchanges
    if not oth.empty:
        o = oth.copy()
        ...
        exch_col = "Exchange" if "Exchange" in o.columns else None
        ...
        for _, row in o.iterrows():
            sym = str(row[sym_col]).strip().upper()
            name = str(row[name_col]).upper() if name_col else ""
            exch = exchange_map.get(str(row[exch_col]).strip().upper(), "OTHER") if exch_col else "OTHER"
            rows.append((sym, name, exch))

    bad_name_terms = [
        " ETF", "ETN", "WARRANT", "RIGHT", " UNIT", "UNITS",
        "PREFERRED", "PREF ", "DEPOSITARY", "DEPOSITORY", "NOTE",
        "BOND", "TRUST", "ACQUISITION CORP. UNIT", "ACQUISITION CORP UNIT",
    ]
    ...
    for sym, name, exch in rows:
        ...
        # Keep only symbols that are reasonably compatible with Yahoo Finance.
        if "$" in sym or "^" in sym or "/" in sym:
            continue

        # Dot classes (BRK.B) need Yahoo's dash notation.
        sym = sym.replace(".", "-")

        if any(term in name for term in bad_name_terms):
            continue

        # Skip obvious special issues by symbol suffix where common.
        # Do not over-filter normal 4/5-letter tickers.
        if sym.endswith(("-WS", "-WT", "-RT", "-U")):
            continue
