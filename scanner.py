...
def _download_symbol_rows(url: str):
    """
    Download Nasdaq Trader symbol-directory files and parse them with csv.DictReader.
    This deliberately avoids pandas parsing quirks seen on some Streamlit Cloud builds.
    """
    import csv
    import io
    ...
    for row in reader:
        ...
        # Footer line such as "File Creation Time: ..."
...
def fetch_all_us_common_stocks() -> List[str]:
    """
    Build a broad US-listed common-stock universe from Nasdaq Trader files.

    Includes Nasdaq + NYSE + NYSE American and other US exchange listings.
    Excludes ETFs, test issues, warrants, rights, units, preferred shares,
    depositary products and obvious debt securities where identifiable.
    """
    nas_rows = _download_symbol_rows(NASDAQ_LISTED)
    oth_rows = _download_symbol_rows(OTHER_LISTED)
    ...
    def add_if_common(sym, name="", etf="N", test_issue="N"):
        sym = str(sym or "").strip().upper()
        name = str(name or "").strip().upper()
        etf = str(etf or "N").strip().upper()
        test_issue = str(test_issue or "N").strip().upper()
        ...
        # Yahoo Finance class-share convention.
        sym = sym.replace(".", "-")

        # Exclude obvious special securities / malformed symbols.
